...
class CustomSidebarLayout extends StatefulWidget {
  ...
  const CustomSidebarLayout({super.key, required this.pageContent, required this.userName, required this.userHandle, required this.userAvatarUrl, required this.onMenuItemTap});
...
class _CustomSidebarLayoutState extends State<CustomSidebarLayout> {
  ...
  int _selectedIndex = 0, _indexBar = 0;
    ...
    return GestureDetector(
      ...
      child: AnnotatedRegion<SystemUiOverlayStyle>(
        value: SystemUiOverlayStyle(
          ...
          statusBarIconBrightness: _isSidebarOpen ? Brightness.light: Brightness.dark,
        ),
        child: Scaffold(
          body: Stack(
            children: [
              AnimatedPadding(
                ...
                child: AnimatedScale(
                  ...
                  child: Transform.translate(
                    offset: Offset(_isSidebarOpen ? 230 : 0, 0),
              ...
              Positioned(
                ...
                child: _isSidebarOpen? IconButton(icon: Icon(Icons.close), color: Colors.white, onPressed: toggleSidebar): GestureDetector(onTap: toggleSidebar, child: Container(height: 45.sp,width: 45,color: Colors.transparent,))
              ),
              ...
              if( _isSidebarOpen&&_selectedIndex == 2)
                Positioned(
                  top: 55.sp,
                  bottom:675.sp,
                  left: 220.sp,
                  child: AnimatedContainer(duration: const Duration(milliseconds: 400), curve: Curves.easeOut, child: Container(width: 20.w, decoration: BoxDecoration(color: Color(primaryColor), border: Border(right: BorderSide(width: 1, color: Color(primaryColor)))))),),
              if (_isSidebarOpen && _selectedIndex == 2)
                Positioned(
                  top: 55.sp,
                  bottom: 675.sp,
                  left: 220.sp,
                  child: AnimatedContainer(duration: const Duration(milliseconds: 400), curve: Curves.easeOut, child: Container(width: 300.w, decoration: BoxDecoration(border: Border(top: BorderSide(width: 0.1.sp, color: Colors.white),right: BorderSide(width: 1, color: Color(primaryColor)))))),
                ),
            ],
          ),
        ),
